...
class CanTransportRuntime {
 public:
  using ReceiveCallback =
      std::function<void(const DeviceFrame&, std::uint64_t sequence)>;

  static Result<CanTransportRuntime> create(SocketCanTransport transport,
                                          ReceiveCallback callback,
                                          std::size_t actuator_slots = 32U);

  CanTransportRuntime(CanTransportRuntime&&) noexcept;
  CanTransportRuntime& operator=(CanTransportRuntime&&) noexcept;
  CanTransportRuntime(const CanTransportRuntime&) = delete;
  CanTransportRuntime& operator=(const CanTransportRuntime&) = delete;
  ~CanTransportRuntime();
 ...
 private:
  ...
  CanTransportRuntime(SocketCanTransport transport, ReceiveCallback callback,
