...
Result<CanTransportRuntime> CanTransportRuntime::create(SocketCanTransport transport,
                                                   ReceiveCallback callback,
                                                   std::size_t actuator_slots) {
  if (!transport.valid() || !callback || actuator_slots == 0U) {
    return Result<CanTransportRuntime>::failure(
  ...
  return Result<CanTransportRuntime>::success(
      CanTransportRuntime(std::move(transport), std::move(callback),
                       std::move(slots)));
}

CanTransportRuntime::CanTransportRuntime(SocketCanTransport transport,
                                   ReceiveCallback callback,
                                   std::vector<std::unique_ptr<Slot>> slots)
...
CanTransportRuntime::CanTransportRuntime(CanTransportRuntime&& other) noexcept
...
CanTransportRuntime& CanTransportRuntime::operator=(CanTransportRuntime&& other) noexcept {
  if (this != &other) {
    stop();
    transport_ = std::move(other.transport_);
    callback_ = std::move(other.callback_);
...
CanTransportRuntime::~CanTransportRuntime() { stop(); }

Result<void> CanTransportRuntime::start() {
...
void CanTransportRuntime::stop() noexcept {
...
Result<void> CanTransportRuntime::stage(std::size_t slot,
...
bool CanTransportRuntime::running() const noexcept {
  return running_.load(std::memory_order_acquire);
}

void CanTransportRuntime::receive_loop(std::stop_token stop) {
...
void CanTransportRuntime::send_loop(std::stop_token stop) {
  while (!stop.stop_requested() && running()) {
